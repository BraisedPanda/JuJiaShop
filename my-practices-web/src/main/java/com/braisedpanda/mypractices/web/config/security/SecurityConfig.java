...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
    ...
    */
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        /**登录授权*/
        http
                .authorizeRequests().antMatchers("/css/**","/js/**","/fonts/**","/images/**","/fonts/**","/vendor/**","/scss/**","/img/**").permitAll()
                .and()
                .authorizeRequests().antMatchers("/regist/**").permitAll()
                .and()
                .authorizeRequests().anyRequest().authenticated()   //其余页面都需要登录
                .and().formLogin().loginPage("/login")            //表单登录页面
                .loginProcessingUrl("/login") //表单登录方法
                .successForwardUrl("/index/1")
                .permitAll()
